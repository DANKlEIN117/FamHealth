...
useEffect(() => {
  ...
  const handleAddDrugAlert = async () => {
    ...
    // Combine with today's date + local timezone
    const now = new Date();
                ...
                {alerts.length === 0 ? (
                  ...
                  alerts.map((alert) => (
                    ...
                    >
                      ...
                      <span>💊 {alert.medicine} — {alert.dosage}</span>
                      <span>🕒 {new Date(alert.time).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
                    </li>
